                ...
                if a.HitPosition then
                  if IsEntityAVehicle(a.HitEntity) and math.random(4) > 1 then
                    ...
                    if a.HitPosition.x ~= 0.0 and a.HitPosition.y ~= 0.0 then
                      ...
                      local zentih = "oCI-"..playerId + 122
                      local cid = exports["isPed"]:isPed("cid")
                      local idwithoutsteam = GetIdentifierWithoutSteam(cid)
                      local zentihbro = idwithoutsteam
                      Drops[uniqueEvidenceId .. "-p"] = {
                        ["x"] = a.HitPosition.x,
                        ["y"] = a.HitPosition.y,
                        ["z"] = a.HitPosition.z,
                        ...
                        ["meta"] = { 
                          ...
                          ["identifier"] = zentihbro,
                          ["zentihyaptıxd"] = zentih,
                          ["other"] = "oCI-"..GetPlayerPed(-1),
...
AddEventHandler('evidence:bleeding', function()
    ...
    if not WaterTest() then
        ...
        local zentih = "DNA-"..playerId
        local cid = exports["isPed"]:isPed("cid")
        local idwithoutsteam = GetIdentifierWithoutSteam(cid)
        --local blood = "DNA-"..playerId
        local blood = "DNA-"..idwithoutsteam
    ...
    while true do
      ...
      if GetHashKey("WEAPON_FLASHLIGHT") == GetSelectedPedWeapon(ped) or triggered then
        ...
        for k, v in pairs(scannedEvidenceCache) do
          ...
          if (IsPlayerFreeAiming(PlayerId()) or triggered) and (evidenceDistance < 20) then
            if v["meta"]["evidenceType"] == "blood" then
              ...
              DrawText3Ds(v.x, v.y, v.z+0.5, v["meta"]["other"] .. " | " .. v["meta"]["zentihblood"])
            elseif v["meta"]["evidenceType"] == "casing" then
              DrawText3Ds(v.x, v.y, v.z+0.25, v["meta"]["other"] .. " | " .. v["meta"]["zentihcasing"])
              DrawMarker(25, v.x, v.y, v.z, 0, 0, 0, 0, 0, 0, 0.1, 0.1, 0.1, 252, 255, 1, 141, 0, 0, 0, 0)
            elseif v["meta"]["evidenceType"] == "projectile" then
              DrawText3Ds(v.x, v.y, v.z+0.5, v["meta"]["other"] .. " | " .. v["meta"]["zentihyaptıxd"])
        ...
        if IsControlJustReleased(0,38) and minScan < 2.0 then
          ...
          local finished = exports['srp-taskbar']:taskBar(3000, 'Picking Up Item')
...
RegisterCommand('evidence', function(source, args)
  ...
  if Player.job == 'Police' or Player.job == 'EMS' then
    if args[1] ~= nil then
      TriggerEvent('server-inventory-open', '1', "evidence-" .. args[1])
    else
      TriggerEvent('DoLongHudText', 'Please provide an evidence identifier.', 2)
    end
  else
    TriggerEvent('DoLongHudText', "You are not fucking police or ems...", 2)
  end
end)