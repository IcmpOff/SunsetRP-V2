...
RegisterNetEvent('srp-ambulancejob:drag')
AddEventHandler('srp-ambulancejob:drag', function(copID)
...
RegisterNetEvent('srp-ambulancejob:putInVehicle')
AddEventHandler('srp-ambulancejob:putInVehicle', function()
...
RegisterNetEvent('srp-ambulancejob:pullOutVehicle')
AddEventHandler('srp-ambulancejob:pullOutVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('srp-ambulancejob:drag')
AddEventHandler('srp-ambulancejob:drag', function(cop)
	IsDragged = not IsDragged
	CopPed = tonumber(cop)
end)

RegisterNetEvent('srp-ambulancejob:un_drag')
AddEventHandler('srp-ambulancejob:un_drag', function(cop)
	IsDragged = not IsDragged
...
RegisterNetEvent('srp-ambulancejob:heal')
AddEventHandler('srp-ambulancejob:heal', function(healType)
    local playerPed = PlayerPedId()
	...
	elseif healType == 'big' then
        ...
        SetEntityHealth(playerPed, newHealth)
        TriggerEvent('srp-hospital:client:RemoveBleed')
        TriggerEvent('srp-hospital:client:ResetLimbs')
    end
    ...
    if closestPlayer == -1 or closestDistance > 2.0 then
		...
		if IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_a", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_b", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_c", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_d", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_e", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_f", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_g", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_h", 3)then
			...
			TriggerEvent('DoLongHudText', 'Revive In Progress')
				...
				TriggerServerEvent('srp-ambulancejob:revive', GetPlayerServerId(closestPlayer))
    ...
    if closestPlayer == -1 or closestDistance > 2.0 then
        TriggerEvent('DoLongHudText', 'There is no player(s) nearby!',2)
			...
			if health > 0 then
				...
				TriggerServerEvent('srp-ambulancejob:heal', GetPlayerServerId(closestPlayer), 'small')
...
AddEventHandler("tp:emsbigheal", function()
    ...
    IsBusy = true
			...
			if health > 0 then
				...
				TriggerServerEvent('srp-ambulancejob:heal', GetPlayerServerId(closestPlayer), 'big')
...
AddEventHandler("tp:emsputinvehicle", function()
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        TriggerServerEvent('srp-ambulancejob:putInVehicle', GetPlayerServerId(closestPlayer))
    else
		TriggerEvent('DoLongHudText', 'There is no player(s) nearby!',2)
    end
end)
...
AddEventHandler("tp:emstakeoutvehicle", function()
    local closestPlayer, closestDistance = GetClosestPlayer()
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        TriggerServerEvent('srp-ambulancejob:pullOutVehicle', GetPlayerServerId(closestPlayer))
    else
		TriggerEvent('DoLongHudText', 'There is no player(s) nearby!',2)
    end
end)
...
AddEventHandler("tp:emsdrag", function()
    local closestPlayer, closestDistance = GetClosestPlayer()
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        TriggerServerEvent('srp-ambulancejob:drag', GetPlayerServerId(closestPlayer))
    else
		TriggerEvent('DoLongHudText', 'There is no player(s) nearby!',2)
    end
end)
...
AddEventHandler("tp:emsundrag", function()
    local closestPlayer, closestDistance = GetClosestPlayer()
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        TriggerServerEvent('srp-ambulancejob:undrag', GetPlayerServerId(closestPlayer))
    else
		TriggerEvent('DoLongHudText', 'There is no player(s) nearby!',2)
    end
end)