...
RegisterNetEvent('srp:playerLoaded')
AddEventHandler('srp:playerLoaded', function(xPlayer)
  PlayerData = xPlayer
end)
...
rootMenuConfig =  {
    {
        ...
        icon = "#globe-europe",
        enableMenu = function()
            isDead = exports["srp-deathmanager"]:GetDeath()
            return not isDead
        end,
    ...
    },
    {
        ...
        enableMenu = function()
            downed = exports["srp-deathmanager"]:GetDeath()
            local LocalPlayer = exports["srp-base"]:getModule("LocalPlayer")
            local Player = LocalPlayer:getCurrentCharacter()
...
newSubMenus = {
    ...
    ['general:civcuff'] = {
        title = "Civ Cuff",
        icon = "#cuffs-cuff",
        functionName = "srp_handcuffs:cuffcheck"
    },
    ['general:civuncuff'] = {
        title = "Civ Uncuff",
        icon = "#cuffs-cuff",
        functionName = "srp_handcuffs:uncuff"
    },    
    ['animations:brave'] = {
        title = "Brave",
    ...
    ['police:frisk'] = {
        ...
        functionName = "srp-policefrisk:menuEvent"
    },
    ['police:pdrevive'] = {
        title = "Revive",
...
local isDead = exports["srp-deathmanager"]:GetDeath()
    if not isDead then
        isDead = true
    else
        isDead = false
...
AddEventHandler('police:tenForteen', function()
	...
	TriggerServerEvent('srp_addons_gcphone:startCall', 'ambulance', '10-14 Medic Down at Coords : ', PlayerCoords, {
...
AddEventHandler('police:tenThirteen', function()
	...
	TriggerEvent('DoLongHudText', 'Distress signal has been sent to available units!')
	TriggerServerEvent('srp_addons_gcphone:startCall', 'police', '10-13 Officer Down at Coords : ', PlayerCoords, {
		PlayerCoords = { x = PedPosition.x, y = PedPosition.y, z = PedPosition.z },
	})
end)
...
RegisterNetEvent('srp_handcuffs:cuffcheck')
AddEventHandler('srp_handcuffs:cuffcheck', function()
    local target, distance = GetClosestPlayer()
    if exports['srp-inventory']:hasEnoughOfItem('cuffs', 1, true) then
        if distance ~= -1 and distance <= 3.0 then
            playerheading = GetEntityHeading(GetPlayerPed(-1))
            playerlocation = GetEntityForwardVector(PlayerPedId())
            playerCoords = GetEntityCoords(GetPlayerPed(-1))
            local target_id = GetPlayerServerId(target)
            TriggerServerEvent('srp-policejob:requesthard', target_id, playerheading, playerCoords, playerlocation)
            Citizen.Wait(1800)
...
RegisterNetEvent('srp_handcuffs:uncuff')
AddEventHandler('srp_handcuffs:uncuff', function()
    if exports['srp-inventory']:hasEnoughOfItem('keya', 1, true) then
        local target, distance = GetClosestPlayer()
        if distance ~= -1 and distance <= 3.0 then
            playerheading = GetEntityHeading(GetPlayerPed(-1))
            playerlocation = GetEntityForwardVector(PlayerPedId())
            playerCoords = GetEntityCoords(GetPlayerPed(-1))
            local target_id = GetPlayerServerId(target)
            TriggerServerEvent('srp-policejob:requestrelease', target_id, playerheading, playerCoords, playerlocation)
            Citizen.Wait(1200)
...
AddEventHandler('FlipVehicle', function()
    ...
    if IsPedInAnyVehicle(ped, false) then vehicle = GetVehiclePedIsIn(ped, false) else vehicle = GetClosestVehicle(coords.x, coords.y, coords.z, 5.0, 0, 71) end
        ...
        local finished = exports["srp-taskbar"]:taskBar(10000,"Flipping Vehicle Over",false,false,playerVeh)
...
AddEventHandler('civputInVehicle', function()
    ...
    if target and distance < 3 then
        TriggerServerEvent('srp-interactions:putInVehicle', GetPlayerServerId(target))
	else
        TriggerEvent('DoLongHudText', 'There is no player(s) nearby!',2)
	end
end)

RegisterNetEvent('srp-interactions:putInVehicle')
AddEventHandler('srp-interactions:putInVehicle', function()
    local playerPed = PlayerPedId()
...
AddEventHandler('civoutOfVehicle', function()
    local target, distance = GetClosestPlayer()
    if target and distance < 3 then
        TriggerServerEvent('srp-interactions:outOfVehicle', GetPlayerServerId(target))
	else
		TriggerEvent('DoLongHudText', 'There is no player(s) nearby!', 2)
	end
end)

RegisterNetEvent('srp-interactions:outOfVehicle')
AddEventHandler('srp-interactions:outOfVehicle', function()
    local playerPed = PlayerPedId()
...
RegisterCommand('scuba', function()
    ...
    if vehFront > 0 then
        if GetVehicleDoorAngleRatio(vehFront, 5) ~= 0.0 then
            ...
            exports["srp_taskbar"]:StartDelayedFunction('Grabbing Scuba Gear', 4000, function()		
	  		    loadAnimDict('anim@narcotics@trash')
    		    TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',0.9, -8, 1900, 49, 3.0, 0, 0, 0)	  		
...
AddEventHandler('pdescort', function()
    ...
    if closestPlayer ~= -1 and closestDistance <= 2.0 then
        ...
        if job == "Police" then
            TriggerServerEvent('srp-policejob:drag', GetPlayerServerId(closestPlayer))
        end
	else
		TriggerEvent('DoLongHudText', 'There is no player(s) nearby!', 2)
	end
