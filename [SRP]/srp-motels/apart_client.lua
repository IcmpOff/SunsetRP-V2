...
AddEventHandler('housing:info:realtor', function(args)
	if modifying then
		...
		if args == "setgarage" then
			...
			TriggerEvent("DoLongHudText","A new garage has been set, there is " .. #modifyingTable["garages"] .. " in total!",91)
			return
		end
...
Citizen.CreateThread(function()
	while true do
		...
		if clothingcoords["x"] ~= 0.0 then
			if #(vector3(clothingcoords["x"],clothingcoords["y"],clothingcoords["z"]-0.3) - plyCoords) < 1.5 then
				nearClothing = true
				DrawText3Ds( clothingcoords["x"],clothingcoords["y"],clothingcoords["z"] , 'Press G to relog or use /outfits to change clothing' )
		...
		if storagecoords["x"] ~= 0.0 then
			if #(vector3(storagecoords["x"],storagecoords["y"],storagecoords["z"]-0.3) - plyCoords) < 1.5 then
				...
				if IsControlJustReleased(2, 38) then
					...
					TriggerEvent("server-inventory-open", "1", "house-"..myhouseid)
...
RegisterNetEvent('housing:outfit')
...
AddEventHandler('housing:findsalecid', function(checkcid,price,house_id,house_model,storage,clothing,garages)
	...
	if buy then
		...
		local Player = exports['srp-base']:getModule('LocalPlayer'):getCurrentCharacter()
...
AddEventHandler('housing:enter', function()
	...
	if modifying or LEO then
		TriggerServerEvent("house:enterhouse",exports['isPed']:isPed('cid'), RealEstate, house_id,house_model,true)
		housenameraid = house_name
	else
		TriggerServerEvent("house:enterhouse",exports['isPed']:isPed('cid'), RealEstate, house_id,house_model,false)
		housenameraid = "none"
	end
...
AddEventHandler("house:entersuccess", function(house_id,house_model,furniture, storage, clothing,garages)
	myhouseid = house_id
	...
	if house_model == 1 then
		if storage ~= nil then
			storagecoords = json.decode(storage)
		end
		if garages ~= nil then
			garagescoords = json.decode(garages)
		end
		if clothing ~= nil then
			clothingcoords = json.decode(clothing)
		end
...
AddEventHandler('inhouse', function(toggle)
	if toggle == true then
		penis = true
	else
		penis = false
			...
			if entryType == 1 then
				...
				then
					...
					elseif shopshit then
						...
						if (#(vector3(curHouseCoords["x"] + 1.279,curHouseCoords["y"] - 7.500,curHouseCoords["z"]-25.62) - playerCoords) < 1.2 ) then
							DrawText3Ds((curHouseCoords["x"] + 1.279),(curHouseCoords["y"] - 7.500),(curHouseCoords["z"]-25.62), '~g~'..Controlkey["housingMain"][2]..'~s~ to leave' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
...
--[[Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1000)
		print("HOUSE: " .. json.encode(curHouseCoords))
		print("MY COORDS: " .. GetEntityCoords(PlayerPedId()))
	end
end)]]--
...
function buildShop(house_id)
	...
	FreezeEntityPosition(PlayerPedId(),true)
	Citizen.Wait(1000)
	RequestModel(GetHashKey("shell_store3"))
	while not HasModelLoaded(GetHashKey("shell_store3")) do
		Citizen.Wait(0)
	end
...
function buildNorth()
	...
	RequestModel(GetHashKey("shell_frankaunt"))
	while not HasModelLoaded(GetHashKey("shell_frankaunt")) do
		Citizen.Wait(0)
	end
...
function buildHouse()
	...
	local generator = { x = curHouseCoords["x"] , y = curHouseCoords["y"], z = curHouseCoords["z"] - 28.0}
	SetEntityCoordsNoOffset(PlayerPedId(), generator.x,generator.y,generator.z+0.5)
    SetEntityHeading(PlayerPedId(), 3.1)
	RequestModel(GetHashKey('shell_lester'))
	while not HasModelLoaded(GetHashKey('shell_lester')) do 
		Wait(0) 
	end

	local building = CreateObject(GetHashKey('shell_lester'),generator.x+2.29760700,generator.y-1.33191200,generator.z+1.26253700,false,false,false)
	FreezeEntityPosition(building,true)

	Citizen.Wait(1600)
	SetEntityCoordsNoOffset(PlayerPedId(),(curHouseCoords["x"] + 1.279),(curHouseCoords["y"] - 7.500),(curHouseCoords["z"]-25.62))
	FreezeEntityPosition(PlayerPedId(),false)
	SetEntityHeading(PlayerPedId(),267.0)
	SetGameplayCamRelativeHeading(0.0)
...
function buildMansion()
	...
	local building = CreateObject(`v_44_shell_2`,generator.x+3.57921200,generator.y+3.70079500,generator.z+0.045,false,false,false)
...
function buildOfficeCentre()
	...
	SetEntityCoords(PlayerPedId(),-139.53950000,-629.07570000,167.82040000)
  	Citizen.Wait(1000)
	local generator = { x = 162.78 , y = -21.89, z = 68.35 - 34.0}
