...
$(document).ready(function () {
  ...
  window.addEventListener('message', function (event) {
    ...
    if (item.response == "openGui") {
      ...
      if (itemList[item.id]) {
        $.post("http://srp-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, true, itemList[item.id].nonStack, item.data, itemList[item.id].weight]));
      } else {
        $.post("http://srp-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, false, itemList[item.id].nonStack, item.data]));
      }
    }
  })
...
function invStack(targetSlot, moveAmount, targetInventory, originSlot, originInventory, purchase, itemCosts, itemidsent, amountmoving, crafting, weapon, amountRemaining) {
  ...
  $.post("http://srp-inventory/stack", JSON.stringify(arr));
};
...
function invMove(targetSlot, originSlot, targetInventory, originInventory, purchase, itemCosts, itemidsent, amountmoving, crafting, weapon) {
  ...
  $.post("http://srp-inventory/move", JSON.stringify(arr));
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory) {
  ...
  $.post("http://srp-inventory/swap", JSON.stringify(arr));
};

function removeCraftItems(itemid, moveAmount) {
  let arr = itemList[itemid].craft
  let amount = moveAmount;
  $.post("http://srp-inventory/removeCraftItems", JSON.stringify([arr, amount]));
};
...
function BuildDrop(brokenSlots) {
  $.post("http://srp-inventory/dropIncorrectItems", JSON.stringify({
    slots: brokenSlots
  }));
}
...
function UpdateSetWeights() {
  ...
  $.post("http://srp-inventory/Weight", JSON.stringify({
    weight: meh
  }));
}
...
function DragToggle(slot,using) {
  ...
  if (slot) {
    ...
    if (occupiedslot == true && dragging == false) {
      ...
      $.post("http://srp-inventory/SlotInuse", JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
  ...
  InventoryLog("Dropped: " + name + " x(" + amountDropped + ") from slot " + slotusing + " of " + inventoryUsedNameText)
   ...
   //$.post('http://srp-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
  //$.post('http://srp-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
  //$.post('http://srp-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(targetSlot, originSlot, inv1, inv2, originAmount, remainingAmount, targetAmount, purchase, itemCosts, itemidsent, moveAmount) {
  ...
  $.post("http://srp-inventory/SlotJustUsed", JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2) {
  // $.post('http://srp-inventory/swapstack', JSON.stringify({
  //   slot1: slot1,
  //   slot2: slot2,
  //   inv1: inv1,
  //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
  ...
  $.post("http://srp-inventory/ServerCloseInventory", JSON.stringify({
    name: TargetInventoryName
  }));
  TargetInventoryName = "none";
  $.post("http://srp-inventory/Close", JSON.stringify({
...
function useitem() {
  ...
  if (dragging != false) {
    ...
    if (inventoryUsedName == PlayerInventoryName) {
      ...
      $.post("http://srp-inventory/invuse", JSON.stringify(arr));
