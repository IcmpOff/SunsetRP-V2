...
RegisterNUICallback('bankGroup', function(data)
    ...
    local lPlayer = exports['srp-base']:getModule('LocalPlayer')
...
AddEventHandler("phone:error", function()
      SendNUIMessage({
        openSection = "error",
        textmessage = "<b>Network Error</b> <br><br> Please contact support if this error persists, thank you for using SRP Phone Services.",
      })   
end)
...
RegisterNUICallback('btnTaskGroups', function()
    local jobs = exports['srp-base']:GetRanks()
...
Citizen.CreateThread(function()
  while true do
    ...
    if IsControlJustPressed(1, 288) then
      if exports['srp-inventory']:hasEnoughOfItem('mobilephone', 1) then
...
function GotPhone()
  local dead = exports["srp-deathmanager"]:GetDeath()
      if not dead then
        openGuiNow()
      else
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    local decrypt = false
    if exports['srp-inventory']:hasEnoughOfItem('decrypterenzo', 1, false) then
      decrypt = true
    end
...
AddEventHandler('phone:addToCall', function(voipchannel)
  ...
  exports['srp-voip']:addPlayerToCall(tonumber(voipchannel))
end)
...
AddEventHandler('phone:removefromToko', function(playerRadioChannel)
  exports['srp-voip']:removePlayerFromCall()
end)
