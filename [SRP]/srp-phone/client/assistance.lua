...
AddEventHandler('phoneGui', function()
  TriggerEvent('jobssystem:current', function(job)
    ...
    if job == nil or job == "None" or job == "trucker" then
...
AddEventHandler('phone:assistClear', function(id,jobsent)
  TriggerEvent('jobssystem:current', function(job)
    if myBlips[id] ~= nil and not myBlips[id].onRoute and (job == jobsent or (exports['isPed']:isPed('job') == "Police" and jobsent == "911")) then
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    -- elseif exports[""]:getModule("LocalPlayer"):getVar("job") == "news" then
      local dist = 0
      if lastBlip.x then 
      ...
      -- if exports[""]:getModule("LocalPlayer"):getVar("job") == "entertainer" then
        Citizen.Wait(300000)
      -- end
    end
      ...
      if (key ~= nil and item ~= nil) then
        ...
        if #(pos - vector3( item.pos.x, item.pos.y, item.pos.z)) < 50.0001 then
          if item.jobType == "ems" then
            -- if exports[""]:getModule("LocalPlayer"):getVar("job") == "ems" then
          ...
          elseif item.jobType == "news" then 
            -- if exports[""]:getModule("LocalPlayer"):getVar("job") == "news" then
              TriggerServerEvent('phone:assistRemove', item.id, item.jobType) -- Send message of clear to others
              clearBlip(item)
              if GetTimeDifference(curTime, item.timestamp) > 2000 then
                if canPay then
