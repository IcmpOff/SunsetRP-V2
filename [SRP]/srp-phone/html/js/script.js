...
$(document).ready(function() {
    ...
    $.post('http://srp-phone/getWeather', JSON.stringify({}));
    ...
    document.onkeyup = function(data) {
        ...
        if (data.which == 27) {
            $.post('http://srp-phone/close', JSON.stringify({}));
        }
    }
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://srp-phone/deleteYP', JSON.stringify({}));
                    break;
                case "racing-create":
                ...
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://srp-phone/newPostSubmit', JSON.stringify({
                    ...
                    break;
                case "group-manage":
                    $.post('http://srp-phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data'), GroupID2: $(this).data('action-data-name'), GroupID3: $(this).data('action-data-rank') }));
                    break;
                case "btnTaskGang":
                    manageGroup = $(this).data('action-data');
                    $.post('http://srp-phone/btnTaskGang', JSON.stringify({}));
                    break;
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        $.post('http://srp-phone/' + action, JSON.stringify({}));
                    break;
                case "dabcoin":
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://srp-phone/race:completed', JSON.stringify({
...
function addGroupManage(group) {
    ...
    for (let i = 0; i < group.employees.length; i++) {
        ...
        console.log(JSON.stringify(employee.name, employee.cid,))
        let employeeEntry = `
        <li>
            <div class="row no-padding">
                <div class="col s12">
        ...
        let groupElement = `
            <li class="collection-item">
                ...
                <a href="#!" class="secondary-content">
                    ...
                    <span class="phone-button" data-action="group-manage" data-action-data="${groups[group].idsent}" data-action-data-name="${groups[group].namesent}" data-action-data-rank="${groups[group].ranksent}" aria-label="Manage" data-balloon-pos="left"><i class="grey-text text-darken-3 fas fa-sign-in-alt fa-2x"></i></span>
                </a>
...
function openBrowser(url) {
    ...
    closePhoneShell();
    $.post('http://srp-phone/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
...
function addVehicles(vehicleData, showCarPayments) {
    ...
    for (let vehicle of Object.keys(vehicleData)) {
        ...
        var vehicleElement = `
            <li>
                <div class="collapsible-header">
                    ...
                    <span class="new badge" style="text-overflow:ellipsis;overflow:hidden;max-width:15ch;white-space: nowrap;" data-badge-caption="">Garage: ${vehicleData[vehicle].garage}</span>
                </div>
                <div class="collapsible-body garage-body">
                    <div class="row">
                        <div class="col s12"> 
                            <ul class="collection">
                                <li class="collection-item"><i class="fas fa-map-marker-alt"></i> Garage: ${vehicleData[vehicle].garage}</li>
                                ...
                                <li class="collection-item"><i class="fas fa-hourglass-half"></i> ${vehicleData[vehicle].lastPayment == 0 ? 'No remaining payments.' : vehicleData[vehicle].lastPayment + ' minutes until payment is due.'}</li>
                                `
        if (vehicleData[vehicle].lastPayment != 0) {
            vehicleElement += `
        ...
        if (vehicleData[vehicle].lastPayment > -2 && vehicleData[vehicle].amountDue > 0)
            vehicleElement += `<button class="waves-effect waves-light btn-small red garage-pay" data-plate="${vehicleData[vehicle].plate}" data-amount="${vehicleData[vehicle].amountDue}"><i class="fas fa-hand-holding-usd"></i> Pay</button> `
...
function addAccountInformation(accountInfo) {
    if (accountInfo) {
        ...
        for (const key of Object(licenses)) {
            ...
            licenseTable += `<tr>
                                <td>${string}</td>
                                <td><i class="${key["status"] == 0 ? "fas fa-check green-text" : "fas fa-check green-text"}"></i></td>
                            </tr>`
        }
...
function addMessage(item) {
    ...
    element.click(function () {
        $.post('http://srp-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: item.msgDisplayName }));
    });
...
function addMessageOther(item) {
    ...
    element.id = item.id;
    element.click(function () {
        $.post('http://srp-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: receiver, clientPhone: item.clientNumber }));
    });
    $(".messages-entries").prepend(element);
}
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://srp-phone/settingsUpdateToko', JSON.stringify({tag: "controlUpdate",controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://srp-phone/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://srp-phone/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://srp-phone/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://srp-phone/racing:map:cancel', JSON.stringify({}));
});

$('#racing-start-tracks').on('change', function (e) {
    let selectedMap = $(this).val();
    if(maps[selectedMap] !== undefined) {
        $.post('http://srp-phone/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://srp-phone/racing:map:load', JSON.stringify({ id: selectedMap}));
...
$('#racing-start').submit(function (e) {
    e.preventDefault();
    $.post('http://srp-phone/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://srp-phone/racing:map:save', JSON.stringify({
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://srp-phone/btnAttemptHouseSale', JSON.stringify({
...
$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://srp-phone/btnTransferHouse', JSON.stringify({
...
$("#group-manage-pay-form").submit(function (e) {
    ...
    $.post('http://srp-phone/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://srp-phone/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://srp-phone/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://srp-phone/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-form").submit(function (e) {
    ...
    $.post('http://srp-phone/newContactSubmit', JSON.stringify({
        name: clean,
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('http://srp-phone/stocksTradeToPlayer', JSON.stringify({
...
$("#twat-form").submit(function (event) {
    event.preventDefault();
    $.post('http://srp-phone/newTwatSubmit', JSON.stringify({
...
$("#call-form").submit(function (event) {
    event.preventDefault();
    $.post('http://srp-phone/callContact', JSON.stringify({
        name: '',
...
$("#new-message-form").submit(function (event) {
    ...
    $.post('http://srp-phone/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://srp-phone/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            setTimeout(function () {
                $.post('http://srp-phone/messages', JSON.stringify({}));
            }, 300);
            break;
    }
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://srp-phone/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://srp-phone/btnEvictHouse', JSON.stringify({}));
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://srp-phone/btnNotifyToggle', JSON.stringify({}));
});

$('.account-information-toggle-pager').click(function () {
    $.post('http://srp-phone/btnPagerToggle', JSON.stringify({}));
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    $.post('http://srp-phone/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://srp-phone/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://srp-phone/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://srp-phone/removeHouseKey', JSON.stringify({
...
$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://srp-phone/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://srp-phone/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-entries').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://srp-phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://srp-phone/btnGarage', JSON.stringify({}));
});

$('.garage-entries').on('click', '.garage-track', function () {
    $.post('http://srp-phone/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://srp-phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate'), amount: $(this).data('amount') }));
    setTimeout(function () {
        $.post('http://srp-phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://srp-phone/loadUserGPS', JSON.stringify({ house_id: $(this).data('house-id'), house_type: $(this).data('house-type') }));
})

$('.contacts-entries, .call-history-entries').on('click', '.btn-contacts-call', function () {
    $.post('http://srp-phone/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
});
...
$('#confirm-modal-accept').click(function (event) {
    $.post('http://srp-phone/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
