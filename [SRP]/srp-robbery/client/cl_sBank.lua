...
RegisterNetEvent('srp-robbery:restartSoon')
AddEventHandler('srp-robbery:restartSoon', function()
    rAllowed = false
end)

RegisterNetEvent('srp-robbery:openSmallBankDoor')
AddEventHandler('srp-robbery:openSmallBankDoor', function(vType)
    print('Opening Door.?')
    ...
    if oDoors[vDoor] == nil then
        if vType == -131754413 then
            oFactor = 90
            TriggerEvent("srp-robbery:smallVaultDrawBoxes" ,vDoor, true)
        else
            TriggerEvent("srp-robbery:smallVaultDrawBoxes" ,vDoor, false)
        end
        oDoors[vDoor] = true
...
RegisterNetEvent('srp-robbery:smallVaultDrawBoxes')
AddEventHandler('srp-robbery:smallVaultDrawBoxes', function(vDoor, vToggle)
...
function DrawSmallBankBoxes(x, y, z, inputType)
    ...
    if distance < 1.0 then
        if IsControlJustPressed(0, 38) then
            ...
            local finished = exports['srp-taskbar']:taskBar(1000, 'Searching')

            if finished == 100 then
                TriggerServerEvent('srp-robbery:sBankBox', cBankId, inputType)
            end
...
RegisterNetEvent('srp-robbery:sBankLoot')
AddEventHandler('srp-robbery:sBankLoot', function()
...
RegisterNetEvent('srp-robbery:securityBlueUsed')
AddEventHandler('srp-robbery:securityBlueUsed', function()
    print('Working.')
    local bId = GetBankId()
    ...
    if vDoor ~= 0 then
        local finished = exports['srp-taskbar']:taskBar(5000, 'Requesting Access')
        TriggerEvent('inventory:removeItem', 'securityblue', 1)
        if finished ~= 100 then
            TriggerEvent('DoLongHudText', 'You cancelled the robbery.', 2)
            return
        end
        TriggerServerEvent('srp-robbery:smallBankAttempt', bId)
    end
...
RegisterNetEvent('srp-robbery:updateBankData')
AddEventHandler('srp-robbery:updateBankData', function(bData)
...
function OpenVaultDoor(vType)
    if vType then
        TriggerEvent('srp-robbery:openSmallBankDoor', 2121050683)
        TriggerEvent('srp-robbery:openSmallBankDoor', -63539571)
    else
        TriggerEvent('srp-robbery:openSmallBankDoor', -131754413)
    end
end
