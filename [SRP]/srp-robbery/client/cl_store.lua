...
RegisterNetEvent('srp-robbery:advLockpickUse')
AddEventHandler('srp-robbery:advLockpickUse', function()
    ...
    if cStoreId ~= false then
        ...
        if #(vector3(sCoords['x'], sCoords['y'], sCoords['z']) - GetEntityCoords(PlayerPedId())) < 3.0 then
            TriggerServerEvent('srp-robbery:attemptSafeRobbery', cStoreId)
            return
        end
        ...
        if rObject then
            ...
            TriggerServerEvent('srp-robbery:attemptRegisterRobbery', cStoreId, rObject)
            return
        end
    end
end)

RegisterNetEvent('srp-robbery:registerRobbery')
AddEventHandler('srp-robbery:registerRobbery', function()
    isLockpicking = true
    local finished = exports['srp-taskbar']:taskBar(2500, 'Robbing Register')
...
RegisterNetEvent('srp-robbery:safeRobbery')
AddEventHandler('srp-robbery:safeRobbery', function()
    isLockpicking = true
    local finished = exports['srp-taskbar']:taskBar(4000, 'Robbing Safe')
...
function storeRegisterLoot()
    TriggerEvent("player:receiveItem", "rollcash", math.random(15))
end
...
function storeSafeLoot()
    if math.random(20) > 15 then
        ...
        TriggerEvent("player:receiveItem", "band", math.random(18))
    end
    ...
    TriggerEvent("player:receiveItem", "rollcash", math.random(14))
end
...
RegisterNetEvent('srp-robbery:regSuccess')
AddEventHandler('srp-robbery:regSuccess', function()
...
RegisterNetEvent('srp-robbery:safeSuccess')
AddEventHandler('srp-robbery:safeSuccess', function()
