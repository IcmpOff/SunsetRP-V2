...
AddEventHandler('admin:getGroup', function()
    ...
    exports.ghmattimysql:execute('SELECT * FROM __users WHERE steam= ?', {identifier}, function(data)
        if data ~= nil then
...
RegisterServerEvent('srp-admin:Cloak')
AddEventHandler('srp-admin:Cloak', function(src, toggle)
    TriggerClientEvent("srp-admin:Cloak", -1, src, toggle)
end)
...
RegisterServerEvent('srp-admin:RaveMode')
AddEventHandler('srp-admin:RaveMode', function(toggle)
    local source = source
    TriggerClientEvent('srp-admin:toggleRave', -1, toggle)
end)
...
AddEventHandler('admin:bringPlayer', function(target)
    ...
    TriggerClientEvent('srp-admin:teleportUser', target, coords.x, coords.y, coords.z)
