...
function SRP.Admin.Menu.DrawTargets(self, cmd, cb)
    ...
    Citizen.CreateThread(function()
        while WarMenu.IsMenuOpened("targetmenu") do
            Citizen.Wait(0)
            for k,v in spairs(SRP._Admin.Players, function(t, a, b) return t[a].src < t[b].src end) do
                if WarMenu.MenuButton("[" .. v.src .. "] " .. v.name, "command") then self:DrawCommand(cmd) cb(v) SRP.Admin:GetCommandData(cmd).drawcommand() return end
            end
...
Citizen.CreateThread(function()
    ...
    local function DrawPlayers()        
        for k,v in spairs(SRP._Admin.Players, function(t, a, b) return t[a].src < t[b].src end) do
            if WarMenu.Button("[" .. v.src .. "] " .. v.name .. (v.sessioned and " - SESSIONED?" or ""), v.status, v.sessioned and {r = 255, g = 0, b = 0, a = 255} or nil) then SRP._Admin.Menu.PlayerOptions = v viewingIp = false viewingLicense = false WarMenu.OpenMenu("aplayeropts") end
        end
    ...
    local function DrawDiscPlayers()
        for k,v in spairs(SRP._Admin.DiscPlayers, function(t, a, b) return t[a].src < t[b].src end) do
            if WarMenu.MenuButton("[" .. v.src .. "] " .. v.name, "aplayeropts") then SRP._Admin.Menu.PlayerOptions = v viewingIp = false end
        end
    ...
    local function DrawPlayerOptions()
        ...
        if options then
            ...
            if SRP.Admin:IsSuperAdmin() then
...
RegisterCommand('setjob', function(source, args)
    if group ~= "user" then
        if args[1] ~= nil then
            for k, v in pairs(exports['srp-base']:GetJobs()) do
                if args[2] == v then
                    TriggerServerEvent('srp-base:setJob', tonumber(args[1]), args[2])
                    for k, v in pairs(exports['srp-base']:GetRanks()) do
                        if v.name == args[2] then
                            local ranks = json.encode(v.ranks)
                            if args[3] ~= nil then
                                TriggerServerEvent('srp-base:setRank', tonumber(args[1]), args[3])
                            else
                    ...
                    local jId = 0
                    local jobs = exports['srp-base']:GetJobs()
                    ...
                    TriggerServerEvent('srp-base:updateJobLogs', args[1], args[3], jId)
                end
            end
        else
