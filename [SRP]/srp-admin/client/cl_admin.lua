...
stop = true
function SRP.Admin.BeginAFK(self, stop)
    if stop then
        afk.isAfk = false
        afk.msgAFK = false
        afk.stringToType = ""
    ...
    afk.event = AddEventHandler("srp-admin:afkStringCheck", function(text)
        if string.lower(text) == afk.stringToType then SRP.Admin:BeginAFK(true) return end
    end)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if curTime - beginTime >= 300000 then TriggerServerEvent("srp-admin:Disconnect", "AFK Kick") return end
        end
    end)
end

function SRP.Admin.AFKCheck(self)
...
RegisterNetEvent("srp-admin:afkStringCheck")
...
AddEventHandler("srp-admin:RemovePlayer", function(data)
    SRP._Admin.DiscPlayers[data.src] = data
    SRP._Admin.Players[data.src] = nil
end)

RegisterNetEvent("srp-admin:RemoveRecent")
AddEventHandler("srp-admin:RemoveRecent", function(data)
    if data ~= nil then
        SRP._Admin.DiscPlayers[data.src] = nil
    end
end)

RegisterNetEvent("srp-admin:AddPlayer")
AddEventHandler("srp-admin:AddPlayer", function(player)
    SRP._Admin.Players[player.src] = player
end)

RegisterNetEvent('srp-admin:teleportUser')
AddEventHandler('srp-admin:teleportUser', function(x, y, z)
    SetEntityCoords(PlayerPedId(), x, y, z)
    TriggerEvent('DoLongHudText', 'You were summoned')
end)

RegisterNetEvent("srp-admin:AddAllPlayers")
AddEventHandler("srp-admin:AddAllPlayers", function(data)
...
RegisterNetEvent("srp-admin:bring")
AddEventHandler("srp-admin:bring", function(target)
    ...
    TriggerServerEvent("srp-admin:bringPlayerServer",pos,target)   
end)
