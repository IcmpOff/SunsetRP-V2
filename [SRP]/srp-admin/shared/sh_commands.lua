...
function cmd.RunCommand(caller, args)
    ...
    --SRP.Admin:SetRank(caller.target, caller.rank)
end
...
function cmd.RunCommand(caller, args)
    ...
    local targPos = caller.retn and cmd.vars.lastPos or NetworkGetPlayerCoords(caller.target.src)
...
function cmd.RunCommand(caller, args)
    ...
    TriggerEvent('DoLongHudText', 'Player : ' .. caller.target.steamid .. " banned for time : " .. caller.time .. " | Reason : " .. caller.reason)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a ban length", "Length: " .. (cmd.vars.time and cmd.vars.time or "No Time Given")) then
        SRP.Admin.Menu:DrawTextInput(cmd.vars.time and cmd.vars.time or "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end

                local timeTable, timeSum, addedTime = SRP.Admin:GetBanTimeFromString(result)
    ...
    if cmd.vars.target and cmd.vars.time then if WarMenu.Button("Ban " .. cmd.vars.target.name) then SRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, reason = cmd.vars.reason, time = cmd.vars.time}) cmd.vars.target = nil cmd.vars.reason = nil cmd.vars.time = nil end end
end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerEvent('DoLongHudText', 'Player with hex : ' .. caller.steamid .. " unbanned from server")
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a Steam ID", "Entered: " .. (cmd.vars.steamid and cmd.vars.steamid or "None")) then 
        SRP.Admin.Menu:ShowTextEntry("Enter a Steam ID", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.steamid then if WarMenu.Button("Unban " .. cmd.vars.steamid) then SRP.Admin:GetCommandData(cmd.command).runcommand({steamid = cmd.vars.steamid}) cmd.vars.steamid = nil end end
end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerServerEvent('srp-admin:Cloak', src, caller.toggle)
end
    ...
    if IsDuplicityVersion() then 
        ...
        AddEventHandler("playerDropped", function()
            ...
            if cmd.vars.cloaked[src] then
                TriggerServerEvent('srp-admin:Cloak', src, false)
...
function cmd.RunCommand(caller, args)
	...
	TriggerEvent('srp-hospital:client:RemoveBleed', caller.target.src) 
	TriggerEvent('srp-hospital:client:ResetLimbs', caller.target.src)
end
...
function cmd.RunCommand(caller, args)
    if not caller.message then return end
    local src = source
    TriggerEvent("srp-admin:runSpawnCommand", caller.message)
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.message then 
        if WarMenu.Button("Spawn model: " .. cmd.vars.message) then 
            SRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) 
        end 
    end
end
...
function cmd.RunCommand(caller, args)
        TriggerServerEvent("srp-admin:RaveMode", cmd.vars.toggle)
end
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Rave Mode: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle SRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
SRP.Admin:AddCommand(cmd)
...
RegisterNetEvent('srp-admin:toggleRave')
AddEventHandler('srp-admin:toggleRave', function(toggle)
    toggles = toggle
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Player Blips: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle SRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Fix Current Vehicle") then 
        ...
        SRP.Admin:GetCommandData(cmd.command).runcommand()
    end
end
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil         
    if WarMenu.Button("Debug: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle SRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
