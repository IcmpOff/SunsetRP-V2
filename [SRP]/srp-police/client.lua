...
function removeDutyRadio()
	if (exports['srp-inventory']:hasEnoughOfItem('radio', 1, false)) then
		print('dick')
...
RegisterNetEvent('srp-policejob:updateBlip') -- BLIPS FOR POLICE & EMS
AddEventHandler('srp-policejob:updateBlip', function()
...
RegisterCommand('sc', function()
	if exports['isPed']:isPed('job') == 'Police' then
	  	...
	  	if distance ~= -1 and distance <= 3.0 then
			...
			playerCoords = GetEntityCoords(GetPlayerPed(-1))
			local target_id = GetPlayerServerId(target)
			TriggerServerEvent('srp-policejob:requestarrest', target_id, playerheading, playerCoords, playerlocation)
		else
			TriggerEvent('DoLongHudText', 'There is no player(s) nearby!', 2)
	  	end
    end
...
AddEventHandler("tp:pdrevive", function()
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
		...
		TriggerServerEvent('srp-ambulancejob:revivePD', GetPlayerServerId(closestPlayer))
...
AddEventHandler('uncuff', function()
	if exports['isPed']:isPed('job') == 'Police' or exports['srp-inventory']:hasEnoughOfItem('shitlockpick', 1, false) then
		local target, distance = GetClosestPlayer()
		if distance ~= -1 and distance <= 3.0 then
		  playerheading = GetEntityHeading(GetPlayerPed(-1))
		  playerlocation = GetEntityForwardVector(PlayerPedId())
...
RegisterNetEvent('srp-policejob:drag')
AddEventHandler('srp-policejob:drag', function(copId)
...
RegisterNetEvent('srp-policejob:putInVehicle')
AddEventHandler('srp-policejob:putInVehicle', function()
...
RegisterNetEvent('srp-policejob:OutVehicle')
AddEventHandler('srp-policejob:OutVehicle', function()
	local playerPed = PlayerPedId()
...
RegisterNetEvent('srp-policejob:getarrested')
AddEventHandler('srp-policejob:getarrested', function(playerheading, playercoords, playerlocation)
	...
	isHandcuffed = true
	IsShackles = false
    TriggerEvent('srp-policejob:handcuff')
    loadAnimDict('mp_arresting')
    TaskPlayAnim(GetPlayerPed(-1), 'mp_arresting', 'idle', 8.0, -8, -1, 49, 0.0, false, false, false)
end)

RegisterNetEvent('srp-policejob:getarrestedhard')
AddEventHandler('srp-policejob:getarrestedhard', function(playerheading, playercoords, playerlocation)
    playerPed = GetPlayerPed(-1)
    SetCurrentPedWeapon(playerPed, GetHashKey('WEAPON_UNARMED'), true) -- unarm player
    local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
    SetEntityCoords(GetPlayerPed(-1), x, y, z)
    SetEntityHeading(GetPlayerPed(-1), playerheading)
	Citizen.Wait(250)
	local finished = exports["srp-taskbarskill"]:taskBar(1200,7)
	if (finished ~= 100) then
		isHandcuffed = true
		IsShackles = true
		TriggerEvent('srp-policejob:handcuff2')
		loadAnimDict('mp_arresting')
		TaskPlayAnim(GetPlayerPed(-1), 'mp_arresting', 'idle', 8.0, -8, -1, 49, 0.0, false, false, false)
...
RegisterNetEvent('srp-policejob:doarrested')
AddEventHandler('srp-policejob:doarrested', function()
	Citizen.Wait(250)
	loadAnimDict('mp_arrest_paired')
...
RegisterNetEvent('srp-policejob:douncuffing')
AddEventHandler('srp-policejob:douncuffing', function()
	Citizen.Wait(250)
	loadAnimDict('mp_arresting')
...
RegisterNetEvent('srp-policejob:getuncuffed')
AddEventHandler('srp-policejob:getuncuffed', function(playerheading, playercoords, playerlocation)
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	SetEntityCoords(GetPlayerPed(-1), x, y, z)
	SetEntityHeading(GetPlayerPed(-1), playerheading)
	Citizen.Wait(250)
