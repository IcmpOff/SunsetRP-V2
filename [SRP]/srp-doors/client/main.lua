...
Citizen.CreateThread(function()
	...
	TriggerServerEvent('srp-doors:getDoorInfo')
end)

RegisterNetEvent('srp-doors:getDoorInfo')
AddEventHandler('srp-doors:getDoorInfo', function(doorInfo, count)
...
RegisterNetEvent('srp-doors:currentlyhacking')
AddEventHandler('srp-doors:currentlyhacking', function(mycb)
			mycb = true
		...
		for i=1, #Config.DoorList do
			...
			if distance < maxDistance then
				...
				if  IsControlJustReleased(1,  38) then
					...
					TriggerServerEvent('srp-doors:updateState', i, doorID.locked)
				end
			end
		end
...
RegisterNetEvent('srp-doors:UseRedKeycard')
AddEventHandler('srp-doors:UseRedKeycard', function()
	...
	if distance < 1 then
		...
		exports["srp_taskbar"]:StartDelayedFunction('Inserting Red Key Card', 5000, function()
			TriggerServerEvent('srp-doors:updateState', 5, false)
...
RegisterNetEvent('srp-doors:UseRedKeycard2')
AddEventHandler('srp-doors:UseRedKeycard2', function()
	local distance = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId(), true), 1846.598, 2604.773, 45.57809, true)
	if distance < 1 then
		TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_ATM", 0, true)
		FreezeEntityPosition(PlayerPedId(), true)
		exports["srp_taskbar"]:StartDelayedFunction('Hacking', 10000, function()
			TriggerServerEvent('srp-doors:updateState', 104, false)
			FreezeEntityPosition(PlayerPedId(), false)
			ClearPedTasks(PlayerPedId())
		end)
	end
...
RegisterNetEvent('srp-doors:UseRedKeycard3')
AddEventHandler('srp-doors:UseRedKeycard3', function()
	local distance = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId(), true), 1819.976, 2604.677, 45.57671, true)
	if distance < 1 then
		TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_ATM", 0, true)
		FreezeEntityPosition(PlayerPedId(), true)
		exports["srp_taskbar"]:StartDelayedFunction('Hacking', 5000, function()
			TriggerServerEvent('srp-doors:updateState', 105, false)
			FreezeEntityPosition(PlayerPedId(), false)
			ClearPedTasks(PlayerPedId())
		end)
	end
...
RegisterNetEvent('srp-doors:UseRedKeycard4')
AddEventHandler('srp-doors:UseRedKeycard4', function()
	local distance = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId(), true), 1804.749, 2615.767, 45.5513, true)
	if distance < 1 then
		TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_ATM", 0, true)
		FreezeEntityPosition(PlayerPedId(), true)
		exports["srp_taskbar"]:StartDelayedFunction('Hacking', 5000, function()
			TriggerServerEvent('srp-doors:updateState', 25, false)
			FreezeEntityPosition(PlayerPedId(), false)
			ClearPedTasks(PlayerPedId())
		end)
	end
	 ...
	 while true do
		...
		if dst <= 1 then
			...
			if dst <= 1 and IsControlJustReleased(0, 38) then
				if exports['srp-inventory']:hasEnoughOfItem('thermite', 1) then
					TriggerEvent('inventory:removeItem', 'thermite', 1)
					FreezeEntityPosition(PlayerPedId(), true)
					local outcome = exports['srp-thermite']:startGame(10,1,8,400)
					if outcome then
						FreezeEntityPosition(PlayerPedId(), false)
						TriggerServerEvent("srp-doors:jailbreak")
					else
						local coords = GetEntityCoords(PlayerPedId())
						exports['srp-thermite']:startFireAtLocation(coords.x, coords.y, coords.z - 1, 10000)
					end
				end
			end
		end
	   ...
	   if dst <= 1 then
		   ...
		   if dst <= 1 and IsControlJustReleased(0, 38) then
			if exports['srp-inventory']:hasEnoughOfItem('thermite', 1) then
				TriggerEvent('inventory:removeItem', 'thermite', 1)
				FreezeEntityPosition(PlayerPedId(), true)
				local outcome = exports['srp-thermite']:startGame(15,1,10,300)
				if outcome then
					FreezeEntityPosition(PlayerPedId(), false)
					TriggerServerEvent("srp-doors:jailbreak2")
				else
					local coords = GetEntityCoords(PlayerPedId())
					exports['srp-thermite']:startFireAtLocation(coords.x, coords.y, coords.z - 1, 10000)
				end
	   ...
	   if dst <= 1 then
		   ...
		   if dst <= 1 and IsControlJustReleased(0, 38) then
				if exports['srp-inventory']:hasEnoughOfItem('thermite', 1) then
					TriggerEvent('inventory:removeItem', 'thermite', 1)
					FreezeEntityPosition(PlayerPedId(), true)
					local outcome = exports['srp-thermite']:startGame(10,1,7,500)
					if outcome then
						FreezeEntityPosition(PlayerPedId(), false)
						TriggerServerEvent("srp-doors:jailbreak3")
					else
						local coords = GetEntityCoords(PlayerPedId())
						exports['srp-thermite']:startFireAtLocation(coords.x, coords.y, coords.z - 1, 10000)
					end
...
RegisterCommand('lockdoors', function()
	local job = exports['srp-base']:getModule("LocalPlayer"):getCurrentCharacter().job
	if job == 'Police' then
		local playerVeh = GetVehiclePedIsIn(PlayerPedId(), false)
		local finished = exports["srp-taskbar"]:taskBar(5000,"Re-Locking All Doors",false,false,playerVeh)
		if finished == 100 then
			for i=1, #Config.PrisonDoors do
				TriggerServerEvent('srp-doors:updateState', i, true)
			end
...
RegisterNetEvent('srp-jailbreak:UnlockAll')
AddEventHandler('srp-jailbreak:UnlockAll', function()
	Citizen.Wait(300000)
	for i=1, #Config.PrisonDoors do
		TriggerServerEvent('srp-doors:updateState', i, false)
	end
end)
...
RegisterNetEvent('srp-doors:setState')
AddEventHandler('srp-doors:setState', function(doorID, state)
	Config.DoorList[doorID].locked = state
end)

RegisterNetEvent('srp-doors:setState2')
AddEventHandler('srp-doors:setState2', function(doorID, state)
