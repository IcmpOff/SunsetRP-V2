...
RegisterServerEvent('srp-hospital:server:RequestBed')
AddEventHandler('srp-hospital:server:RequestBed', function()
    for k, v in pairs(beds) do
        if not v.taken then
            v.taken = true
            bedsTaken[source] = k
            TriggerClientEvent('srp-hospital:client:SendToBed', source, k, v)
            return
        end
    end
...
RegisterServerEvent('srp-hospital:server:RPRequestBed')
AddEventHandler('srp-hospital:server:RPRequestBed', function(plyCoords)
    ...
    for k, v in pairs(beds) do
        ...
        if distance < 3.0 then
            if not v.taken then
                v.taken = true
                foundbed = true
                TriggerClientEvent('srp-hospital:client:RPSendToBed', source, k, v)
                return
            else
...
RegisterServerEvent('srp-hospital:server:EnteredBed')
AddEventHandler('srp-hospital:server:EnteredBed', function()
    local src = source
    ...
    TriggerClientEvent('srp-hospital:client:FinishServices', src)
end)

RegisterServerEvent('srp-hospital:server:LeaveBed')
AddEventHandler('srp-hospital:server:LeaveBed', function(id)
    beds[id].taken = false
end)