...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('srp-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('srp-hospital:client:SyncBleed')
AddEventHandler('srp-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('srp-hospital:client:FieldTreatLimbs')
AddEventHandler('srp-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('srp-hospital:client:FieldTreatBleed')
AddEventHandler('srp-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('srp-hospital:client:ReduceBleed')
AddEventHandler('srp-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('srp-hospital:client:ResetLimbs')
AddEventHandler('srp-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('srp-hospital:client:RemoveBleed')
AddEventHandler('srp-hospital:client:RemoveBleed', function()
    isBleeding = 0
...
RegisterNetEvent('srp-hospital:client:UsePainKiller')
AddEventHandler('srp-hospital:client:UsePainKiller', function(tier)
    if tier < 4 then
...
RegisterNetEvent('srp-hospital:client:UseAdrenaline')
AddEventHandler('srp-hospital:client:UseAdrenaline', function(tier)
    if tier < 4 then
        onDrugs = 180 * tier
    end
