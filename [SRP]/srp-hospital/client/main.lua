...
function LeaveBed()
    ...
    TriggerServerEvent('srp-hospital:server:LeaveBed', bedOccupying)
...
RegisterNetEvent('srp-hospital:client:RPCheckPos')
AddEventHandler('srp-hospital:client:RPCheckPos', function()
    TriggerServerEvent('srp-hospital:server:RPRequestBed', GetEntityCoords(PlayerPedId()))
end)

RegisterNetEvent('srp-hospital:client:RPSendToBed')
AddEventHandler('srp-hospital:client:RPSendToBed', function(id, data)
    bedOccupying = id
...
RegisterNetEvent('srp-hospital:client:SendToBed')
AddEventHandler('srp-hospital:client:SendToBed', function(id, data)
    bedOccupying = id
    bedOccupyingData = data
    ...
    Citizen.CreateThread(function ()
        ...
        TriggerServerEvent('srp-hospital:server:EnteredBed')
    end)
end)

RegisterNetEvent('srp-hospital:client:FinishServices')
AddEventHandler('srp-hospital:client:FinishServices', function()
                ...
                if hspDist < 2 then
                    ...
                    if IsControlJustReleased(0, 54) then
                        if (GetEntityHealth(PlayerPedId()) < 201) or (IsInjuredOrBleeding()) then
                            ...
                            exports["srp_taskbar"]:StartDelayedFunction('Checking Credentials', 1500, function()
                                TriggerEvent('srp-ambulancejob:revive')
                                ClearPedTasks(PlayerPedId())
                                Citizen.Wait(1000)
                                TriggerServerEvent('srp-hospital:server:RequestBed')
                            end)
                        else
