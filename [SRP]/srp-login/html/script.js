...
function sendNuiMessage(data){
    $.post("http://srp-login/nuiMessage", JSON.stringify(data));
}
...
function buildCharacterSlots(characters){
    ...
    for (var k in characters){
        ...
        var job = "<div class='job'><span class='entry'>Job:</span><br> " + char.job + "</div><br><br>";
        ...
        $("#characters").append(numSlot + title + "<div id='cdata' class='scroll'>" + name + dob + gender + phone + cash + bank + job + story + "</div>" + buttons + "</div>");
    }
