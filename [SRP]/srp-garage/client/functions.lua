...
SpawnVehicle = function(data,recuperar)
    ...
    if vehicleProps.garage == cachedData["currentGarage"] then
	    ...
	    CloseMenu()
	    local gameVehicles = exports["srp-base"]:GetVehicles()
        ...
        Citizen.Wait(100)
        TriggerServerEvent('srp-garage:modifystate', vehicleProps.vehicle, 0, nil)
        HandleCamera(cachedData["currentGarage"])
        if recuperar then
            TriggerEvent('DoLongHudText', 'You paid $200 to recover your vehicle!')
            TriggerServerEvent('srp-garage:pay')
        end
        ClearMenu()
    else
...
SaveInGarage = function(garage)
    ...
    if vehicle == 0 then
        ...
        deleteCar(vehicle)
        TriggerServerEvent('srp-garage:modifystate', vehicleProps, 1, garage)
        CloseMenu()
    end
end
...
SetVehicleProperties = function(vehicle, vehicleProps)
    exports['srp-base']:SetVehProps(vehicle, vehicleProps)
    ...
    exports["srp-carhud"]:SetFuel(vehicle, round(vehicleProps.fuelLevel))
...
GetVehicleProperties = function(vehicle)
    if DoesEntityExist(vehicle) then
        local vehicleProps = exports['srp-base']:FetchVehProps(vehicle)
        ...
        vehicleProps.fuelLevel = exports['srp-carhud']:GetFuel(vehicle)
