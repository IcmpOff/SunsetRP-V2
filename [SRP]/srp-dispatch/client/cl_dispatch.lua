    ...
    if pNotificationData ~= nil then
        if pNotificationData.recipientList then
            for key, value in pairs(pNotificationData.recipientList) do
                if key == exports['isPed']:isPed("job") and value and not disableNotifications then
                    ...
                    if exports['isPed']:isPed("job") ~= "news" then
                        SendNUIMessage({
                            mId = "notification",
                            eData = pNotificationData
                        })
                        sendNewsBlip(pNotificationData)
                    elseif exports['isPed']:isPed("job") == "news" then
                        if exports["srp-inventory"]:getQuantity("scanner") > 0 then
                            local newsObject = {}
                    ...
                    if(pNotificationData.playSound and exports['isPed']:isPed('job') ~= "news" and not disableNotificationSounds) then
...
RegisterNetEvent("srp:playerBecameJob")
AddEventHandler("srp:playerBecameJob", function(job, name, notify)
    currentJob = job
end)
...
Citizen.CreateThread(function()
	while true do
        Citizen.Wait(0)
        if IsControlJustReleased(0, 121) and exports['isPed']:isPed("job") == "Police" then
...
function addBlip(data)
    ...
    myBlips[blipId] = {
      ...
      blip = blip,
      id = blipId,
      jobType = exports['isPed']:isPed('job')
    }
