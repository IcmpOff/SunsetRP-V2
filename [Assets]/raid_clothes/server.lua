...
AddEventHandler("raid_clothes:insert_character_current",function(data, characterId)
...
AddEventHandler("raid_clothes:insert_character_face",function(data, characterId)
    if not data then return end
    local src = source
...
AddEventHandler("raid_clothes:get_character_face",function(characterId)
    local src = source
...
AddEventHandler("raid_clothes:get_character_current",function(characterId)
    local src = source
...
AddEventHandler("raid_clothes:retrieve_tats", function(char)
    local src = source
	exports.ghmattimysql:execute("SELECT * FROM playerstattoos WHERE identifier = @identifier", {['identifier'] = char}, function(result)
        if(#result == 1) then
			...
			local tattooValue = "{}"
			exports.ghmattimysql:execute("INSERT INTO playerstattoos (identifier, tattoos) VALUES (@identifier, @tattoo)", {['identifier'] = char, ['tattoo'] = tattooValue})
...
AddEventHandler("raid_clothes:set_tats", function(tattoosList, char)
	local src = source
	exports.ghmattimysql:execute("UPDATE playerstattoos SET tattoos = @tattoos WHERE identifier = @identifier", {['tattoos'] = json.encode(tattoosList), ['identifier'] = char})
end)


RegisterServerEvent("raid_clothes:get_outfit")
AddEventHandler("raid_clothes:get_outfit",function(slot, characterId)
...
AddEventHandler("raid_clothes:set_outfit",function(slot, name, data, characterId)
    if not slot then return end
    local src = source
...
AddEventHandler("raid_clothes:remove_outfit",function(slot, cid)
...
AddEventHandler("raid_clothes:list_outfits",function(cid)
    local src = source
    local slot = slot
    local name = name
...
AddEventHandler("clothing:checkIfNew", function(cid)
    local src = source
