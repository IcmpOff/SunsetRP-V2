...
function Save(save)
    if save then
        ...
        TriggerServerEvent("raid_clothes:insert_character_current", data, Player.id)
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            TriggerServerEvent("raid_clothes:insert_character_face", data, Player.id)
            TriggerServerEvent("raid_clothes:set_tats", currentTats, Player.id)
        end
    else
        LoadPed(oldPed)
    end
...
AddEventHandler("raid_clothes:hasEnough", function(menu)
    if menu == "tattoomenu" then
        local LocalPlayer = exports["srp-base"]:getModule("LocalPlayer")
        local Player = LocalPlayer:getCurrentCharacter()
        TriggerServerEvent("raid_clothes:retrieve_tats", Player.id)
...
AddEventHandler("raid_clothes:setclothes", function(data,alreadyExist)
    ...
    local function setDefault()
        Citizen.CreateThread(function()
            ...
            Wait(5000)
            if not exports["srp-inventory"]:hasEnoughOfItem("idcard",1,false) then
            ...
            TriggerEvent("DoLongHudText","Theres the Hotel, finally, go inside and check it out. Be sure to save your clothing in the dresser by typing '/outfitadd 1 New Outfit' when you are near it. (P for Phone, F1 for Actions, K for inventory - most shops will show you what to do, also, you can use gurgle on your phone for more help!)")
    ...
    local LocalPlayer = exports["srp-base"]:getModule("LocalPlayer")
    local Player = LocalPlayer:getCurrentCharacter()
    TriggerServerEvent("raid_clothes:get_character_face", Player.id)
    TriggerServerEvent("raid_clothes:retrieve_tats", Player.id)
	TriggerServerEvent("Police:getMeta")
    TriggerEvent("e-blips:updateAfterPedChange",exports["isPed"]:isPed("myjob"))
end)
...
AddEventHandler('raid_clothes:outfits', function(pAction, pId, pName)
    ...
    elseif pAction == 2 then
        TriggerServerEvent("raid_clothes:remove_outfit",pId, Player.id)
    elseif pAction == 3 then 
        TriggerEvent('item:deleteClothesDna')
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("raid_clothes:get_outfit", pId, Player.id)
    else
